...
class CustomRNNCell(nn.Module):
    ...
    def forward(self, inputs, prev_output):
        ...
        omega_i = prev_output @ self.select_w
        diag_w = torch.diag_embed(omega_i)
        linear_input = torch.matmul(diag_w, self.ones_frequency)
        diag_delta = torch.diag_embed(prev_output @ self.select_delta)

        mu_pos = torch.sum(F.softplus(linear_input + \
                                          b_plus) * w_plus, dim=2)
        mu0_pos = torch.sum(F.softplus(torch.zeros_like(linear_input) + \
                                          b_plus) * w_plus, dim=2)
        mu_pos = mu_pos - mu0_pos

        mu_neg = torch.sum(F.softplus(-linear_input + \
                                           b_minus) * w_minus, dim=2)
        mu0_neg = torch.sum(F.softplus(torch.zeros_like(linear_input) + \
                                           b_minus) * w_minus, dim=2)
        mu_neg = mu0_neg - mu_neg

        action_nonconstrained = torch.where(omega_i>=0, mu_pos, mu_neg)
...
class MonotonicRNN:
    ...
    def train(self, Pi):
        ...
        for i in range(self. episodes):
            Pi_tensor = torch.tensor(Pi, dtype=torch.float32).unsqueeze(1)
    ...
    def plot_loss_record(self, filename="Training_Loss.png"):
        np.save("softplus_loss_record.npy", np.array(self.loss_record))
        print(f"[✓] Loss record saved to softplus_loss_record.npy")          
        plt.figure()
    ...
    def result_vis(self, filepath, figure_name, Pi, gen_id, SimulationLength):
        ...
        np.save(f"softplus_total_deviation.npy", Record_loss)
    ...
    def plot_learned_policies(self, filepath, figure_name):
        ...
        omega_vals = np.linspace(-0.02, 0.02, 2000).reshape(-1, 1)
...
def plot_loss_record_from_file(filename="Training_Loss.png"):
    loss_record = np.load("softplus_loss_record.npy")
    print(f"[✓] Loaded loss record from softplus_loss_record.npy")
