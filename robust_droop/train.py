...
class CustomRNNCell(nn.Module):
    def __init__(self, env, action_units, internal_units):
        ...
        self.w_temp0 = nn.Parameter(torch.rand(action_units, internal_units) * 0.1)
    ...
    def forward(self, inputs, prev_output):
        ...
        action_nonconstrained = linear_input * w
...
class MonotonicRNN:
    ...
    def train(self, Pi):
        ...
        for i in range(self. episodes):
            ...
            loss_action = self.env.Penalty_action * torch.sum(pow(action_all, 2)) / self.batch_size
    ...
    def export_controller_weights(self, filename):
        weights = {
            "w_temp0": self.model.w_temp0.detach().cpu().numpy()
        }
    ...
    def plot_learned_policies(self, filepath, figure_name):
        self.load(filepath)
        self.model.eval()
        omega_vals = np.linspace(-0.08, 0.08, 2000).reshape(-1, 1)
